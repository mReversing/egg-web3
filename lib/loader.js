...
module.exports = app => {
  const { engine, options, providerOptions, ignoreCase, ...config } = app.config.web3;
  let provider;
  let web3;

  if (ignoreCase && Web3.utils.toChecksumAddress) Web3.utils.toChecksumAddress = address => address.toLowerCase();
    ...
    if (type === 'ws') { // @see https://github.com/ethereum/web3.js/issues/1354#issuecomment-389722913
      ...
      Provider.prototype.send = function () {
        if (
          ...
          provider = new Provider(config.rpcUrl, providerOptions);
  ...
  return web3 = new Web3(provider, options);
};
